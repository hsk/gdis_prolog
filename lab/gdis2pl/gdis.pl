...
pop((_, D, _, [(G, _,  I)|S]), succ(G, D, I, S)).
pop((_, D, _,             []), fail(D)).
...
uni(M,E,T,T2,M2) :-
	unify((T,T2),E,E2), M=([_|G],D,_,[(Sg, _, I)|S]), M2=succ(G, D, -1,[(Sg,E2,I)|S])
  ; pop(M,M2).
...
step1(M,[atom('!')           |G], D, -1, [(G2,E,_)|S], R) :- step(succ(G, D, -1, [(G2, E, -2)|S]), R).
step1(M,[pred(',',  [U,V])   |G], D, -1, S, R) :- step(succ([U,V|G], D, -1, S), R).
step1(M,[pred(';',  [U,V])   |G], D, -1, S, R) :- e(S, E), step(succ([U|G], D, -1, [([V|G], E, -1)|S]), R).
...
step1(M,                   [T|G], D,  I, S, R) :- nth0(I, D, pred(':-', [T2, T3])),!,
                                                  e(S,E),
                                                  length(S, Len1), Len is Len1 + 1,
                                                  gen_t(Len,T2,T2d),
                                                  I1 is I + 1,
                                                  ( unify((T, T2d), E, E2), gen_t(Len,T3, T3d),
                                                    step(succ([T3d|G], D, -1, [([T|G], E2, I1)|S]),R)
                                                  ; step(succ([T| G], D, I1, S), R)
                                                  ).
step1(M,                      G , D,  I, S, R) :- pop(M,M2), step(M2,R).

gen_t(Len, pred(N,Ts), pred(N,Ts2)) :- maplist(gen_t(Len), Ts, Ts2).
gen_t(Len, var((N,_)), var((N,Len))).
gen_t(Len, T, T).
...
process(D, T, D2) :- prove(([T], D, -1, [([],[],-2)]), D2).
