...
step(succ(G,D,I,S), R2) :- (flag(trace,Tr,Tr),Tr=1,format("i=~p G=~p S=~p~n",[I,G,S]);!),!,step1((G,D,I,S), G,D,I,S, R2).
step(R, R).
...
repl(D) :-
    ...
    ;Y='h', help,!, repl(D)
    ;Y='t', flag(trace,T,1-T),(T=0,S='on';S='off'),format('Tracing ~s~n',[S]),!,repl(D)
