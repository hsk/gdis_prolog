...
% prolog
in_list(V,[V|_]).
in_list(V,[_|Vs]) :- in_list(V,Vs).
env_show_test_(Vs,((N,L),_)) :- in_list(var((N,L)),Vs).
env_show_add_(E, ((N,_),T),R) :- deref(E,T,T2), syntax_show(T2,T3),format(atom(R),"~s=~s",[N,T3]).
env_show(Vs,E,R) :-
  include(env_show_test_(Vs),E,E2),
...
prove(Vs,M,D2) :- solve(M, R),
  ( R = fail(D2)
  ; R = succ(G, D, I, S),
    e(S,V), env_show(Vs, V, X), write(X),nl,
    ((S=[]; I= -2), write("Yes.\n"), D2=D
...
getVars(var(V),[var(V)]).
getVars(pred(_,Ls),R) :- maplist(getVars,Ls,Rs),append(Rs,R).
getVars(_,[]).

process(D, T, D2) :-
  getVars(T,Vs), prove(Vs,([T], D, -1, [([],[],1,-2)]), D2).
